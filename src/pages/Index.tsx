...
const content = {
  fr: {
    ...
    certTitle: "Certifications & perfectionnement", certs: ["Cours d’anglais · Graffingstone Training College, Nairobi · depuis mars 2026", "AWS · Introduction to Amazon DynamoDB", "AWS · Getting Started with AWS Systems Manager", "AWS · Building a Machine Learning-Ready Organization"],
  ...
  en: {
    ...
    certTitle: "Certifications & development", certs: ["English course · Graffingstone Training College, Nairobi · since March 2026", "AWS · Introduction to Amazon DynamoDB", "AWS · Getting Started with AWS Systems Manager", "AWS · Building a Machine Learning-Ready Organization"],
    ...
    <main className="min-h-screen overflow-x-hidden bg-background px-3 py-3 text-foreground sm:px-6 sm:py-8 lg:py-14">
      <div className="mx-auto max-w-7xl overflow-hidden rounded-lg border border-border bg-card shadow-panel">
        <header className="relative z-50 flex min-h-20 items-center justify-between px-5 sm:px-8 md:px-12">
          <a href="#top" className="flex items-center gap-3 font-display text-sm font-extrabold uppercase"><span className="flex h-9 w-9 items-center justify-center rounded-md bg-primary text-primary-foreground">AC</span><span className="hidden sm:inline">Ariel Christ</span></a>
          ...
          <div className="flex items-center gap-2">
            <div className="flex rounded-md border border-border bg-muted p-1" aria-label="Language selector">{(["fr", "en"] as const).map((item) => <Button key={item} onClick={() => switchLanguage(item)} variant={language === item ? "outline" : "ghost"} size="sm" className="h-8 px-3 font-mono text-[11px] uppercase shadow-none" aria-pressed={language === item}>{item}</Button>)}</div>
            <Button variant="ghost" size="icon" className="lg:hidden" onClick={() => setMenuOpen(!menuOpen)} aria-label={t.menu}>{menuOpen ? <X /> : <Menu />}</Button>
          </div>
          {menuOpen && <nav className="absolute inset-x-0 top-full border-y border-border bg-card px-5 py-3 shadow-panel lg:hidden">{t.nav.map((item, index) => <a key={item} onClick={() => setMenuOpen(false)} href={["#about", "#projects", "#path", "#skills"][index]} className="block border-b border-border py-3 text-sm font-semibold text-muted-foreground last:border-0">{item}</a>)}</nav>}
        </header>
        ...
        <section id="top" className="grid border-y border-border lg:grid-cols-12">
          <div className="flex flex-col justify-center px-6 py-14 sm:px-10 lg:col-span-7 lg:px-16 lg:py-20 xl:px-20">
            <p className="mb-6 flex w-fit items-center gap-2 rounded-full bg-accent px-3 py-1.5 font-mono text-[10px] uppercase text-accent-foreground"><span className="h-2 w-2 rounded-full bg-success" />{t.role}</p>
            <h1 className="font-display text-4xl font-extrabold leading-[1.04] sm:text-6xl xl:text-7xl">Ariel Christ<br /><span className="text-secondary">Austhen Ngato</span></h1>
        ...
        <section id="about" className="section-shell grid gap-10 lg:grid-cols-[0.9fr_1.1fr]"><div><p className="section-label">{t.profileLabel}</p><h2 className="section-title mt-5">{t.profileTitle}</h2></div><div><p className="text-lg leading-8 text-muted-foreground">{t.profileText}</p><div className="mt-9 grid gap-4 sm:grid-cols-2"><div className="info-line"><MapPin /><span>{t.location}</span></div><div className="info-line"><GraduationCap /><span>Master Data Science & IA</span></div></div></div></section>
        ...
        <section id="contact" className="m-3 rounded-lg bg-primary px-6 py-14 text-center text-primary-foreground sm:m-8 sm:px-10 lg:flex lg:items-center lg:justify-between lg:px-16 lg:py-16 lg:text-left"><div><p className="font-mono text-xs uppercase text-blue-foreground">{t.contactLabel}</p><h2 className="mt-5 max-w-3xl font-display text-3xl font-bold leading-tight sm:text-4xl">{t.contactTitle}</h2><p className="mt-4 text-primary-foreground/70">{t.contactText}</p></div><div className="mt-8 flex flex-wrap justify-center gap-3 lg:mt-0 lg:justify-end"><Button asChild size="lg" variant="secondary" className="h-12 rounded-md px-6"><a href="mailto:arielchristngato@outlook.fr">{t.write}<Mail /></a></Button><Button asChild size="lg" variant="outline" className="h-12 rounded-md border-primary-foreground/30 bg-transparent px-5 text-primary-foreground hover:bg-primary-foreground hover:text-primary"><a href="tel:+254700477731"><Phone />+254 7 004 777 31</a></Button></div></section>
